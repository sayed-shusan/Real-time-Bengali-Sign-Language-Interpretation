...
classifier = tf.estimator.Estimator(model_dir="tmp/cnn_model2", model_fn=cnn_tf.cnn_model_fn)
...
def put_splitted_text_in_blackboard(splitted_text):
    y = 200
    blackboard = np.zeros((480, 640, 3), dtype=np.uint8)
    for text in splitted_text:
        fontpath = "./SolaimanLipi_20-04-07.ttf"
        # fontpath = "./AdorshoLipi-Normal.ttf"
        font = ImageFont.truetype(fontpath, 32)
        img_pil = Image.fromarray(blackboard)
        draw = ImageDraw.Draw(img_pil)
        # draw.text((4, y), text, fill=(255, 255, 255, 255))
        draw.text((4, y), text, font=font, fill=(255, 255, 255, 255))
        blackboard = np.array(img_pil)
        # cv2.putText(blackboard, text, (4, y), font, 2, (255, 255, 255))
        y += 50
    return blackboard
...
def recognize():
    ...
    while True:
        ...
        blackboard = put_splitted_text_in_blackboard(splitted_text)
